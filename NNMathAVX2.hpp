...
// ---------------------------------------------------------
// 1. CUSTOM AVX2/FMA SIMD KERNELS (O(N) operations)
// ---------------------------------------------------------
...
void avx2_add(const double* a, const double* b, double* res, size_t n) {
...
void avx2_sub(const double* a, const double* b, double* res, size_t n) {
    //here auto vectorization is better or else i will have to manually unroll the loops n shi but i think link time optimization is needed
    for (size_t i = 0; i < n; i++)
    {
        res[i] = a[i] - b[i];
...
void avx2_div(const double* a, const double* b, double* res, size_t n) {
    //here auto vectorization is better or else i will have to manually unroll the loops n shi but i think link time optimization is needed
    for (size_t i = 0; i < n; i++)
    {
        res[i] = a[i] / b[i];
...
void avx2_add_scalar(const double* a, double val, double* res, size_t n) {
    //here auto vectorization is better or else i will have to manually unroll the loops n shi but i think link time optimization is needed
    for (size_t i = 0; i < n; i++)
    {
        res[i] = a[i] + val;
...
void avx2_sub_scalar(const double* a, double val, double* res, size_t n) {
    //here auto vectorization is better or else i will have to manually unroll the loops n shi but i think link time optimization is needed
    for (size_t i = 0; i < n; i++)
    {
        res[i] = a[i] - val;
...
void avx2_mul_scalar(const double* a, double val, double* res, size_t n) {
    //here auto vectorization is better or else i will have to manually unroll the loops n shi but i think link time optimization is needed
    for (size_t i = 0; i < n; i++)
    {
        res[i] = a[i] * val;
...
inline double avx2_dot_product(const double* a, const double* b, size_t len)
{
...
inline void avx2_hadamard_product(const double* a, const double* b, double* res, size_t len) {
    //here auto vectorization is better or else i will have to manually unroll the loops n shi but i think link time optimization is needed
...
inline void avx2_fma(const double* a, const double* b, double scalar, double* res, size_t len) {
    __m256d v_scalar = _mm256_set1_pd(scalar);
    size_t vec_len = len & ~3ULL; // Process in chunks of 4
    size_t i = 0;
    ...
    for (; i < vec_len; i += 4) {
        ...
        // _mm256_fmadd_pd(a, b, c) performs: (a * b) + c
        // Here: (va * v_scalar) + vb
        __m256d v_res = _mm256_fmadd_pd(va, v_scalar, vb);
 ...
 * Uses AVX2 to process 4 elements at a time.
 * Formula: x * (1 + (1/9)x^2 + (1/945)x^4) / (1 + (4/9)x^2 + (1/63)x^4)
 * * param data Pointer to the double array to be modified.
 * param len Number of elements in the array.
 */
inline void avx2_tanh_pade(double* data, size_t len) {
    // Define constants for the Padé approximation coefficients
    const __m256d v_one = _mm256_set1_pd(1.0);
    const __m256d v_c1_num = _mm256_set1_pd(1.0 / 9.0);      // 1/9
    const __m256d v_c2_num = _mm256_set1_pd(1.0 / 945.0);    // 1/945
    const __m256d v_c1_den = _mm256_set1_pd(4.0 / 9.0);      // 4/9
    const __m256d v_c2_den = _mm256_set1_pd(1.0 / 63.0);     // 1/63
    ...
    for (; i < vec_len; i += 4) {
        // Load 4 doubles
        __m256d v_x = _mm256_loadu_pd(data + i);

        // Calculate x^2 and x^4 once to reuse
        __m256d v_x2 = _mm256_mul_pd(v_x, v_x);
        __m256d v_x4 = _mm256_mul_pd(v_x2, v_x2);

        // Numerator: 1 + (1/9)x^2 + (1/945)x^4
        // Using FMA: (v_c2_num * v_x4) + ((v_c1_num * v_x2) + 1.0)
        __m256d v_num_terms = _mm256_fmadd_pd(v_c1_num, v_x2, v_one);
        __m256d v_num = _mm256_fmadd_pd(v_c2_num, v_x4, v_num_terms);
        
        // Final numerator: x * (numerator_poly)
        v_num = _mm256_mul_pd(v_x, v_num);

        // Denominator: 1 + (4/9)x^2 + (1/63)x^4
        __m256d v_den_terms = _mm256_fmadd_pd(v_c1_den, v_x2, v_one);
        __m256d v_den = _mm256_fmadd_pd(v_c2_den, v_x4, v_den_terms);

        // Result: Numerator / Denominator
        __m256d v_res = _mm256_div_pd(v_num, v_den);

        // Store back in place
        _mm256_storeu_pd(data + i, v_res);
    }
...
inline void avx2_outer_product(const double* u, size_t m, const double* v, size_t n, double* res)
{
    size_t vec_len_n = n & ~3ULL; // Process columns in chunks of 4

    for (size_t i = 0; i < m; ++i) {
        // Broadcast the scalar u[i] to all 4 elements of the SIMD register.
        // We do this once per row.
        __m256d v_u = _mm256_set1_pd(u[i]);
        ...
        for (; j < vec_len_n; j += 4) {
            ...
            __m256d v_v = _mm256_loadu_pd(v + j);
            
            // Multiply u[i] * v[j...j+3]
            __m256d v_res = _mm256_mul_pd(v_u, v_v);
            
            // Store the result directly into the result matrix
            _mm256_storeu_pd(row_start + j, v_res);
        }

        // Scalar cleanup for remaining elements in the row (if n is not a multiple of 4)
        for (; j < n; ++j) {
            row_start[j] = u[i] * v[j];
        }
    }
 ...
 */
inline void avx2_sech2_from_tanh(const double* tanh_vals, double* res, size_t len) {
    const __m256d v_one = _mm256_set1_pd(1.0);
    size_t vec_len = len & ~3ULL; // Process in chunks of 4
    size_t i = 0;
    ...
    for (; i < vec_len; i += 4) {
        // Load 4 tanh values
        __m256d v_t = _mm256_loadu_pd(tanh_vals + i);

        // Calculate 1.0 - (tanh * tanh)
        // _mm256_fnmadd_pd(a, b, c) performs: -(a * b) + c
        // This is faster than separate multiply and subtract instructions.
        __m256d v_res = _mm256_fnmadd_pd(v_t, v_t, v_one);
